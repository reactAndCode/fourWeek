"use client"

import { useState, useEffect } from "react"
import { Button } from "@/components/ui/button"
import { CheckCircle2, Loader2 } from "lucide-react"
import { getMemoByTab, upsertMemo } from "@/lib/api/memos"
import { useAuth } from "@/hooks/useAuth"

const BASE_MEMO_TAB = "Base Memo"

export function BaseMemo() {
  const { user } = useAuth()
  const [content, setContent] = useState("")
  const [originalContent, setOriginalContent] = useState("")
  const [isLoading, setIsLoading] = useState(false)
  const [isSaving, setIsSaving] = useState(false)
  const [hasChanges, setHasChanges] = useState(false)

  // 컴포넌트 마운트 시 메모 로드
  useEffect(() => {
    if (user) {
      loadBaseMemo()
    }
  }, [user])

  // 변경사항 추적
  useEffect(() => {
    setHasChanges(content !== originalContent)
  }, [content, originalContent])

  const loadBaseMemo = async () => {
    if (!user) return

    setIsLoading(true)
    try {
      const memo = await getMemoByTab(user.id, BASE_MEMO_TAB)
      const loadedContent = memo?.content || ""
      setContent(loadedContent)
      setOriginalContent(loadedContent)
    } catch (error) {
      console.error('기준 메모 로드 실패:', error)
      setContent("")
      setOriginalContent("")
    } finally {
      setIsLoading(false)
    }
  }

  const handleSave = async () => {
    if (!user || !hasChanges) return

    setIsSaving(true)
    try {
      await upsertMemo(user.id, BASE_MEMO_TAB, "기준 메모", content)
      setOriginalContent(content)
      console.log('기준 메모 저장 성공')
    } catch (error) {
      console.error('기준 메모 저장 실패:', error)
      alert('저장에 실패했습니다.')
    } finally {
      setIsSaving(false)
    }
  }

  const handleCancel = () => {
    if (hasChanges && confirm('변경사항을 취소하시겠습니까?')) {
      setContent(originalContent)
    }
  }

  return (
    <div className="bg-white rounded-xl shadow-sm h-full flex flex-col">
      {/* 헤더: 제목 + 저장 상태 + 버튼들 */}
      <div className="px-8 pt-8 pb-4 flex items-center justify-between border-b border-gray-200">
        <h2 className="text-xl font-bold">기준 메모</h2>

        {/* 우측: 저장 상태 + 버튼들 */}
        <div className="flex items-center gap-3">
          {/* 저장 상태 */}
          <CheckCircle2 className={`h-5 w-5 ${!hasChanges && !isSaving ? 'text-green-600' : 'text-gray-400'}`} />
          <span className={`text-sm ${!hasChanges && !isSaving ? 'text-green-600' : 'text-gray-500'}`}>
            {isSaving ? '저장 중...' : !hasChanges ? '저장 완료' : '저장되지 않음'}
          </span>

          {/* 버튼들 */}
          <Button
            variant="outline"
            onClick={handleCancel}
            className="px-4"
            disabled={!hasChanges || isSaving}
          >
            취소
          </Button>
          <Button
            onClick={handleSave}
            className="px-4 bg-blue-500 hover:bg-blue-600"
            disabled={!hasChanges || isSaving}
          >
            {isSaving ? (
              <>
                <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                저장 중...
              </>
            ) : (
              '저장'
            )}
          </Button>
        </div>
      </div>

      {/* 컨텐츠 영역 */}
      <div className="flex-1 p-8">
        {isLoading ? (
          <div className="w-full h-full flex items-center justify-center border border-gray-300 rounded-lg">
            <Loader2 className="h-8 w-8 animate-spin text-gray-400" />
          </div>
        ) : (
          <textarea
            value={content}
            onChange={(e) => setContent(e.target.value)}
            placeholder="모든 날짜에 공통으로 참고할 내용을 입력하세요..."
            className="w-full h-full p-4 text-sm border border-gray-300 rounded-lg resize-none focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            disabled={isSaving}
          />
        )}
      </div>
    </div>
  )
}
