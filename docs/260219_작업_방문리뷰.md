...
### 아. Vercel 빌드 오류 해결 (2026-02-19 추가)
- **오류 원인**: `app/api/send-fortune/route.ts`에서 `Resend` 클라이언트를 최상단(Top-level)에서 초기화할 때, 빌드 시점에 API 키가 없어 빌드 실패 발생
- **해결 방법**: `Resend` 클라이언트 초기화를 `POST` 핸들러 내부로 이동하여 지연 초기화(Lazy Initialization) 방식으로 수정
- **검증**: 기타 OpenAI 및 Replicate 관련 API 라우트들도 핸들러 내부에서 초기화되고 있음을 확인하여 추가 잠재적 오류 예방
